...
class FTDistiller(nn.Module):
    """
    Distiller for 'FT' (Kim et al. 2019).
    FT: Factor Transfer
    Uses factor transfer for knowledge distillation.
    """
    def __init__(
        self,
        ...
        self.factor_weight = cfg.get("factor_weight", 1.0)
    ...
    def forward(self, x, y, tau=None):
        """
        FT forward pass:
        1) teacher => factors + features + logits
        2) student => factors + features + logits  
        3) CE + Factor Transfer + Feature KD
        """
        with torch.no_grad():
            ...
            t_factor = t_out.get("factor", None)  # teacher factors
        
        s_dict, s_logit, _ = self.student(x)
        s_feat = s_dict.get("feat_2d", None)  # student features
        s_factor = s_dict.get("factor", None)  # student factors
        ...
        # Factor Transfer Loss (FT 핵심)
        factor_loss = 0.0
        if t_factor is not None and s_factor is not None:
            # Factor alignment loss
            if t_factor.shape == s_factor.shape:
                # L2 loss between teacher and student factors
                factor_loss = F.mse_loss(s_factor, t_factor)
            else:
                # Adaptive pooling if shapes don't match
                if len(t_factor.shape) == 4 and len(s_factor.shape) == 4:
                    # Adaptive average pooling to match sizes
                    target_size = t_factor.shape[2:]
                    s_factor_adapted = F.adaptive_avg_pool2d(s_factor, target_size)
                    
                    # Handle different channel dimensions
                    if s_factor_adapted.shape[1] != t_factor.shape[1]:
                        min_channels = min(s_factor_adapted.shape[1], t_factor.shape[1])
                        s_factor_adapted = s_factor_adapted[:, :min_channels]
                        t_factor_adapted = t_factor[:, :min_channels]
                    else:
                        t_factor_adapted = t_factor
                    
                    factor_loss = F.mse_loss(s_factor_adapted, t_factor_adapted)
                else:
                    # For non-4D tensors, try to match dimensions
                    if len(t_factor.shape) == 2 and len(s_factor.shape) == 2:
                        # For 2D tensors, use linear projection if needed
                        if t_factor.shape[1] != s_factor.shape[1]:
                            # Simple approach: use global average pooling
                            if len(s_factor.shape) == 4:
                                s_factor_adapted = F.adaptive_avg_pool2d(s_factor, 1).flatten(1)
                            else:
                                s_factor_adapted = s_factor.flatten(1)
                            factor_loss = F.mse_loss(s_factor_adapted, t_factor)
        ...
        total_loss = (
            ...
            self.factor_weight * factor_loss +
    ...
    def train_distillation(
        ...
        FT training loop:
          ...
          - Factor transfer + feature distillation
        """
        self.to(device)
